...
class MTREncoder(nn.Module):
    ...
    def apply_local_attn(self, x, x_mask, x_pos, num_of_neighbors):
        ...
        local_attn = self.self_attn_layers[-1].attn_mat
        attn_mat = torch.zeros((x_stack_full.shape[0], 16)).cuda() 
        attn_inds = -torch.ones((x_stack_full.shape[0], 16)).cuda().int()
        attn_mat[x_mask_stack] = local_attn
        attn_inds[x_mask_stack] = index_pair
        self.attn_mat = attn_mat.view(batch_size, N, 16)
        self.attn_ind = attn_inds.view(batch_size, N, 16)
        self.x_mask = x_mask
    ...
    def forward(self, batch_dict):
        ...
        batch_dict['map_polylines'] = map_polylines
        batch_dict['map_polylines_mask'] = map_polylines_mask
