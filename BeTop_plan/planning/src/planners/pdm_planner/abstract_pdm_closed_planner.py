...
class AbstractPDMClosedPlanner(AbstractPDMPlanner):
    ...
    def __init__(
        ...
        # config parameters
        print(trajectory_sampling, proposal_sampling)
    ...
    def _get_full_closed_loop_trajectory(
        self,
        ...
        trajectory_list = self._generator.generate_full_trajectory()
        trajs = []
        for i in range(proposal_scores.shape[0]):
            trajectory = trajectory_list[i]
            future_time_points = [
                trajectory.start_time + TimeDuration.from_s(0.1) * (i + 1)
                for i in range(40)
            ]
            ...
            planner_trajectory = planner_trajectory[..., :3]  # drop values
            trajs.append(planner_trajectory)

        trajs = np.array(trajs)
        return trajs, proposal_scores
