...
from src.planners.pdm_planner.observation.pdm_object_manager_pred import (
    PDMObjectManager,
)
...
class PDMObservationPred:
    ...
    def update(
        ...
        self._object_manager = self._get_object_manager(ego_state, observation, pred, token, behave_occ)
        ...
        # print(self._observation_samples + self._observation_sample_res, self._observation_sample_res)
        for sample in horizons:
            if has_dynamic_object:
                dynamic_object_coords_t = dynamic_object_dxy[:, :, sample]
    ...
    def _get_object_manager(
        self, ego_state: EgoState, observation: Observation, preds=None, pred_tokens=None, behave_occ=None
    ) -> PDMObjectManager:
        """
        ...
        if pred_tokens is not None:
            token_dict  = {pred_tokens[i]: i for i in range(pred_tokens.shape[0])}
        for object in observation.tracked_objects:
            if (
                ...
                continue
            if pred_tokens is not None:
                if object.track_token in pred_tokens:
                    if behave_occ is not None:
                        if behave_occ[token_dict[object.track_token]] < 0.3:
                            object_manager.add_object(object)
                            continue
                    object_manager.add_object(object, preds[token_dict[object.track_token]])
                else:
                    object_manager.add_object(object)
            else:
                object_manager.add_object(object)
