...
class PDMGenerator:
    ...
    def generate_full_trajectory(
        ...
        for proposal_idx in range(len(self._proposal_manager)):
            ...
            single_time_list = copy.deepcopy(self._time_point_list)
            single_curr_time = copy.deepcopy(self._time_point_list[-1])
            lateral_batch_idcs = [proposal_idx]
            ...
            for time_idx in range(
                ...
                single_curr_time += TimePoint(int(self._sample_interval * 1e6))
                single_time_list.append(single_curr_time)
            ...
            ret_list.append(InterpolatedTrajectory(ego_states))
        return ret_list
